...
class bm_request {
	...
	public function get_user_custom_fields_data( $id, $visibility = false ) {
		...
		$gids             = get_user_meta( $id, 'bm_group', true );
	...
	public function bm_check_field_exist( $gid, $type, $signup = false ) {
		$dbhandler = new bm_DBhandler();
	...
	public function bm_get_currency_symbol() {
			  $dbhandler = new bm_DBhandler();
		$currency        = $dbhandler->get_global_option_value( 'bm_paypal_currency', 'USD' );
		switch ( $currency ) {
			case 'USD':
				$sign = '&#36;';
				break;
	...
	public function bm_encrypt_decrypt_pass( $action, $string ) {
		$output         = false;
		...
		if ( isset( $fields ) && ! empty( $fields ) ) {
			foreach ( $fields as $field ) {
				...
				if ( ( $field->field_type == 'file' || $field->field_type == 'user_avatar' ) && isset( $files[ $field_key ] ) && ! empty( $files[ $field_key ]['name'][0] ) ) {
					$field_options = maybe_unserialize( $field->field_options );
					$allowed_ext   = ( ( $field_options['allowed_file_types'] != '' ) ? $field_options['allowed_file_types'] : $dbhandler->get_global_option_value( 'bm_allow_file_types', 'jpg|jpeg|png|gif' ) );
					// $current_file_type = '';
					if ( $field->field_type == 'user_avatar' ) {
							$allowed_ext       = 'jpg|jpeg|png|gif';
							$require_imagesize = $this->bm_get_minimum_requirement_user_avatar();
					}
                                        else
                                        {
				...
				$error = apply_filters( 'bm_frontend_server_validation', $error, $post );
				return $error;
	}

	public function profile_magic_show_captcha( $option ) {
				 $dbhandler    = new bm_DBhandler();
		$enable_recaptcha      = $dbhandler->get_global_option_value( 'bm_enable_recaptcha' );
	...
	public function profile_magic_get_error_message( $error_code, $textdomain ) {
		switch ( $error_code ) {
			...
			case 'incorrect_password':
				...
				if ( function_exists( 'is_wpe' ) ) :
					$forgot_pwd_url = '/wp-login.php?action=lostpassword&wpe-login=true';
								...
								$forget_password_url = $this->profile_magic_get_frontend_url( 'bm_forget_password_page', site_url( $forgot_pwd_url ) );
			...
			case 'payment_pending':
				...
				$message = sprintf( $url, $this->bm_get_repayment_url(sanitize_text_field(wp_unslash($_REQUEST['id']) )) );
				break;
			...
			case 'loginrequired':
								...
								$login_page_url = $this->profile_magic_get_frontend_url( 'bm_user_login_page', site_url( '/wp-login.php' ) );
		...
		if ( $user_info == false ) {
			...
			switch ( $field ) {
				...
				case 'user_pass':
						...
						$value = $this->bm_encrypt_decrypt_pass( 'decrypt', $pass );
					break;
				case 'user_nicename':
				...
				case 'source_domain':
					...
					break;
				case 'profile_link':
						$value = $this->bm_get_user_profile_url( $userid );
					break;
				case 'site_name':
						$value = get_bloginfo( 'name' );
					break;
				case 'group_name':
						$gids = get_user_meta( $userid, 'bm_group', true );
						$gid  = $this->pg_filter_users_group_ids( $gids );
	...
	public function profile_magic_get_user_attachment( $value, $key = '' ) {
		...
		foreach ( $values as $fileid ) {
					$attachment_html .= '<span class="bm_frontend_attachment">';
			...
			if ( $key != '' ) {
				$attachment_html .= '<a class="removebutton"><span onClick="bm_remove_attachment(this,\'' . $key . '\',' . $fileid . ')" class="remove">' . esc_html__( 'Remove', 'profilegrid-user-profiles-groups-and-communities' ) . '</span></a>';
			}
	...
	public function profile_magic_edit_user_attachment( $value, $key = '' ) {
		...
		foreach ( $values as $fileid ) {
			?>
					<span class="bm_frontend_attachment
					<?php
					if ( $key != '' ) {
						echo ' bm_edit_attachment'; }
					?>
					">
						<span class="attachment_icon"> <?php echo wp_get_attachment_link( $fileid, 'full', false, true, false ); ?></span>
						<a class="bm_removebutton"><span onClick="bm_remove_attachment(this, '<?php echo esc_attr( $key ); ?>', <?php echo esc_attr( $fileid ); ?>)" class="remove"><?php esc_html_e( 'Remove', 'profilegrid-user-profiles-groups-and-communities' ); ?></span></a>
					</span>
				<?php
		}
		unset( $values );
	...
	public function profile_magic_get_from_email() {
		...
		if ( $dbhandler->get_global_option_value( 'bm_enable_smtp' ) == 1 ) {
			...
			$from_name     = $dbhandler->get_global_option_value( 'bm_from_email_name', get_bloginfo( 'name' ) );
			$email_address = $from_name . ' <' . $dbhandler->get_global_option_value( 'bm_from_email_address', get_option( 'admin_email' ) ) . '>';
		}
	...
	public function profile_magic_get_admin_email() {
			   $dbhandler      = new bm_DBhandler();
				$email_address = '';
				$email_array   = maybe_unserialize( $dbhandler->get_global_option_value( 'bm_admin_email' ) );
	...
	public function profile_magic_frontend_registration_request( $post, $files, $server, $gid, $fields ) {
		...
		$user_id               = $dbhandler->bm_add_user( $user_name, $password, $user_email, $user_role );
		...
		$newpass = $this->bm_encrypt_decrypt_pass( 'encrypt', $password );
		update_user_meta( $user_id, 'user_pass', $newpass );
		update_user_meta( $user_id, 'rm_user_status', '1' );
		$this->bm_update_user_custom_fields_data( $post, $files, $server, $gid, $fields, $user_id );
		if ( $is_paid_group == '0' ) {
	...
	public function bm_admin_notification_message_html( $post, $gid, $fields, $exclude = array() ) {
		$html = '';
		if ( ! empty( $fields ) ) {
			$html .= '<table>';
			foreach ( $fields as $field ) {
	...
	public function bm_get_minimum_requirement_user_avatar() {
		$dbhandler     = new bm_DBhandler();
	...
	public function bm_update_user_custom_fields_data( $post, $files, $server, $gid, $fields, $user_id ) {
			  $dbhandler     = new bm_DBhandler();
				$pmsanitizer = new bm_sanitizer();
		if ( isset( $fields ) && ! empty( $fields ) ) :
			foreach ( $fields as $field ) {
				...
				if ( $field->field_type == 'file' || $field->field_type == 'user_avatar' ) {
					...
					if ( is_array( $filefield ) ) {
						...
						for ( $i = 0; $i < $arr_count; $i++ ) {
							...
							if ( $field->field_type == 'user_avatar' ) {
								$minimum_requirement = $this->bm_get_minimum_requirement_user_avatar();
							} else {
				...
				if ( $field->field_type == 'user_url' ) {
					...
					if ( $field->field_type == 'file' ) {
						if ( $value != '' ) {
							...
							if ( $user_attachment != '' && $dbhandler->get_global_option_value( 'bm_allow_multiple_attachments' ) == 1 ) {
				...
				do_action( 'bm_update_user_profile', $user_id );
		endif;
	}
	public function bm_get_user_avatar( $userid ) {
		$avatar = get_avatar(
			$userid,
			274,
			'',
	...
	public function bm_get_repayment_url( $uid ) {
		$gids             = get_user_meta( $uid, 'bm_group', true );
				$gid      = $pmrequests->pg_filter_users_group_ids( $gids );
		$registration_url = $this->profile_magic_get_frontend_url( 'bm_registration_page', '' );
	...
	public function bm_get_user_redirect( $gid ) {
			  $dbhandler = new bm_DBhandler();
	...
	public function bm_get_user_date_query( $get ) {
		$date_query = array();
	...
	public function bm_get_user_meta_query( $get ) {
		...
		if ( isset( $get['bm_user_label'] ) && $get['bm_user_label'] != '' ) {
			if ( $get['bm_user_label'] != 'none' ) {
									$meta_query_array[] = array(
										'key'     => 'bm_user_label',
										'value'   => sprintf( ':"%s";', $get['bm_user_label'] ),
										'compare' => 'like',
									);
			} else {
					 $meta_query_array[] = array(
		...
		if ( isset( $get['connection'] ) && $get['connection'] != '' ) {
			$meta_query_array[] = array(
				'key'     => 'bm_' . $get['connection'] . '_connected',
				'value'   => 1,
				'compare' => '=',
			);
		}
		...
		if ( isset( $get['match_field'] ) && $get['match_field'] != '' && isset( $get['field_value'] ) && $get['field_value'] != '' ) {
			...
			if ( in_array( $get['match_field'], array( 'user_url', 'user_email', 'user_nicename', 'user_login' ) ) ) {
                ...
                {
                    $mqry = $this->bm_get_default_search_field_meta_query($get);
                    if($mqry)
                    {
        ...
        public function bm_get_default_search_field_meta_query($get)
        {
            $dbhandler = new bm_DBhandler();
            $search = '';
            if(isset($get['bm_search']))
            {
                $search = $get['bm_search'];
            }
            ...
            $bm_default_search_field = $dbhandler->get_global_option_value('bm_default_search_field','first_name');
            switch($bm_default_search_field)
            {
                case 'first_name':
                case 'last_name':
                    $meta_query_array = array(
					'key'     => $bm_default_search_field,
					'value'   => $get['bm_search'],
					'compare' => 'LIKE',
				);
                    break;
	...
	public function bm_get_user_advance_search_meta_query( $get ) {
		 $meta_query_array = array();
		$search_string     = esc_attr( trim( $get['bm_search'] ) );
		...
		if ( isset( $get['match_fields'] ) && isset( $get['bm_search'] ) && $get['bm_search'] != '' ) {
	...
	public function bm_get_hide_users_array() {
		 $dbhandler       = new bm_DBhandler();
		$id               = array();
		$allowhiddenusers = $dbhandler->get_global_option_value( 'bm_allow_user_to_hide_their_profile', '' );
		if ( $allowhiddenusers == 1 ) {
			$args = array(
				'meta_query' => array(
					array(
						'key'     => 'bm_hide_my_profile',
						'value'   => '1',
						'compare' => '=',
					),
				),
	...
	public function bm_get_frontend_user_meta( $uid, $gid, $group_leader, $view = '', $section = '', $exclude = '' ) {
			  $dbhandler = new bm_DBhandler();
		$data            = array();
		if ( is_array( $gid ) ) {
			$gid_array = $gid;
	...
	public function bm_get_backend_user_meta( $uid, $gid, $group_leader, $view = '', $section = '', $exclude = '' ) {
			   $dbhandler = new bm_DBhandler();
		$data             = array();
		if ( is_array( $gid ) ) {
			$gid_array = $gid;
	...
	public function profile_magic_set_group_leader( $gid ) {
		...
		if ( isset( $group_leader_name ) && $group_leader_name != '' && $is_group_leader != 0 ) {
			...
			update_user_meta( $userid, 'bm_group', $gid );
		}
	}
	...
	public function profile_magic_get_cover_image( $uid, $class = '' ) {
		$dbhandler = new bm_DBhandler();
		$imageid   = $this->profile_magic_get_user_field_value( $uid, 'bm_cover_image' );
		$path      = plugins_url( '../public/partials/images/default-cover.jpg', __FILE__ );
		$avatarid  = $dbhandler->get_global_option_value( 'bm_default_cover_image', '' );
	...
	public function profile_magic_get_bm_theme_name() {
		 $dirname               = array();
		$bm_theme_path          = plugin_dir_path( __FILE__ ) . '../public/partials/themes/';
		$wp_theme_dir           = get_stylesheet_directory();
		$override_bm_theme_path = $wp_theme_dir . '/profilegrid-user-profiles-groups-and-communities/themes/';
		if ( file_exists( $bm_theme_path ) ) {
			foreach ( glob( $bm_theme_path . '*', GLOB_ONLYDIR ) as $dir ) {
				  $dirname[] = basename( $dir );
			}
		}

		if ( file_exists( $override_bm_theme_path ) ) {
			foreach ( glob( $override_bm_theme_path . '*', GLOB_ONLYDIR ) as $dir2 ) {
	...
	public function profile_magic_get_bm_theme_path() {
		 $dirname               = array();
		$bm_theme_path          = plugin_dir_path( __FILE__ ) . '../public/partials/themes/';
		$wp_theme_dir           = get_stylesheet_directory();
		$override_bm_theme_path = $wp_theme_dir . '/profilegrid-user-profiles-groups-and-communities/themes/';
	...
	public function bm_update_user_activation_code( $user_id ) {
		if ( (int) $user_id ) {
			$pass            = wp_generate_password( 10, false );
			$activation_code = md5( $pass );
			update_user_meta( $user_id, 'bm_activation_code', $activation_code );
		}
	}

	public function bm_create_user_activation_link( $user_id, $activation_code ) {
		...
		$user_data_enc                  = rawurlencode( $this->bm_encrypt_decrypt_pass( 'encrypt', $user_data_json ) );
		$user_activation_link           = admin_url( 'admin-ajax.php' ) . '?action=bm_activate_user_by_email&user=' . $user_data_enc;
	...
	public function bm_get_display_name( $uid, $labels = false ) {
	...
	public function bm_five_star_review_banner() {
		?>
<div class="bm_five_star_Banner">
	
	<div class="bm_five_star_Banner_wrap">
	...
	public function bm_get_user_all_threads( $uid, $limit = false, $active = false ) {
		if ( $uid ) {
			$dbhandler  = new bm_DBhandler();
	...
	public function bm_create_message( $sid, $rid, $content ) {
		$mid = 'false';
		if ( $sid != '' && $rid != '' ) {
			$dbhandler = new bm_DBhandler();
			$pmemail = new bm_Emails;
			...
			if ( $alreay_sent == '' ) {
				$dbhandler->update_global_option_value( $option, 0 );
                         ...
                         $send_email = $dbhandler->get_global_option_value('bm_unread_message_notification','0');
			 if($send_email=='1')
			 {
                            $pmemail->bm_send_unread_message_notification($sid,$rid);
			 }
		}
		if ( $mid == 'false' ) {
			return false;
			...
			$this->bm_update_thread_time( $tid, 2 );
			// $this->bm_update_thread_status($tid,2); //RID is sent for status of thread
			return $tid;
		}

	}

	public function bm_edit_message( $rid, $mid, $content ) {
		$current_user = wp_get_current_user();
		$sid          = $current_user->ID;
                $dbhandler    = new bm_DBhandler();
                $identifier   = 'MSG_CONVERSATION';
	...
	public function bm_update_thread_time( $tid, $status = '' ) {
		$dbhandler  = new bm_DBhandler();
		$identifier = 'MSG_THREADS';
		$title      = array();
		$data       = array(
	...
	public function bm_update_thread_status( $tid, $status ) {
		$dbhandler  = new bm_DBhandler();
		$identifier = 'MSG_THREADS';
	...
	public function bm_filter_deleted_threads( $threads ) {
	...
	public function bm_filter_deleted_message( $messages, $tid ) {
		$dbhandler         = new bm_DBhandler();
		$uid               = wp_get_current_user()->ID;
	...
	public function bm_get_user_online_status( $uid ) {
		 // return get_user_meta( $uid, 'bm_login_status', true );
		return $this->is_user_online( $uid );
	}

	public function bm_get_profile_slug_by_id( $uid ) {
		 $slug = $uid;
		$slug  = apply_filters( 'profile_magic_get_filter_slug_by_id', $slug, $uid );
		return $slug;
	}

	public function bm_get_uid_from_profile_slug( $slug ) {
		 $uid = $slug;
		$uid  = apply_filters( 'profile_magic_get_filter_uid_by_slug', $uid, $slug );
		return $uid;
	}

	public function bm_get_user_profile_url( $uid ) {
		$slug        = $this->bm_get_profile_slug_by_id( $uid );
		$profile_url = $this->profile_magic_get_frontend_url( 'bm_user_profile_page', '' );
	...
	public function bm_get_user_edit_profile_url( $uid, $gid ) {
		$slug        = $this->bm_get_profile_slug_by_id( $uid );
		$profile_url = $this->profile_magic_get_frontend_url( 'bm_user_profile_page', '' );
	...
	public function bm_is_eligable_for_edit_profile( $uid, $gid, $user_id ) {
		$dbhandler    = new bm_DBhandler();
		$row          = $dbhandler->get_row( 'GROUPS', $gid );
		$group_leader = username_exists( $row->leader_username );
		if ( is_user_logged_in() ) {
			$filter_user_id = $this->bm_get_profile_slug_by_id( $uid );
	...
	public function bm_user_ip() {
		...
		return apply_filters( 'bm_user_ip', $ip );
	}
	...
	public function pg_get_default_avtar_src() {
		...
		$dbhandler = new bm_DBhandler();
		$avatarid  = $dbhandler->get_global_option_value( 'bm_default_avatar', '' );
		if ( $avatarid == '' ) {
			if ( ! is_admin() ) {
				 $bm_avatar = $path . '/partials/images/default-user.png';
			} else {
				 $bm_avatar = $path . '../admin/partials/images/default-user.png';
			}

			
		} else {
			$bm_avatar = wp_get_attachment_url( $avatarid );
		}
	...
	public function pg_get_default_cover_image_src() {
		...
		if ( $avatarid == '' ) {
			if ( ! is_admin() ) {
				 ...
				 $bm_avatar = $path . '/partials/images/default-cover.jpg';
			} else {
				 $bm_avatar = $path . '../admin/partials/images/default-cover.jpg';
			}
	...
	public function profile_magic_check_profile_access_permission( $profile_id ) {
		$current_user_id         = get_current_user_id();
			$access_level        = $this->profile_magic_get_user_field_value( $profile_id, 'bm_profile_privacy' );
			$profile_user_groups = $this->profile_magic_get_user_field_value( $profile_id, 'bm_group' );
			$profile_user_group  = $this->pg_filter_users_group_ids( $profile_user_groups );

			$current_user_groups = $this->profile_magic_get_user_field_value( $current_user_id, 'bm_group' );
		...
		if ( ! is_array( $current_user_group ) ) {
			...
			$pmfriends       = new bm_Friends_Functions();
			$pmrequests      = new bm_request();
			$access          = false;
	...
	public function is_pg_dashboard_page() {
		...
		$pg_pages = array(
			'bm_general_settings',
			'bm_security_settings',
			'bm_user_settings',
			'bm_email_settings',
			'bm_third_party_settings',
			'bm_import_options',
			'bm_export_options',
			'bm_seo_settings',
			'bm_upload_settings',
			...
			'bm_analytics',
			'bm_membership',
			'bm_content_restrictions',
			'bm_bbpress_settings',
			'bm_group_fields_settings',
			'bm_display_name_settings',
			'bm_front_end_groups_settings',
			'bm_geolocation_settings',
			'bm_group_photos_settings',
			'bm_group_wall_settings',
			'bm_mailchimp',
			'bm_add_mailchimp_list',
			'bm_mailchimp_settings',
			'bm_uid_changer_settings',
			'bm_woocommerce_settings',
			'bm_social_connect_settings',
		);
	...
	public function bm_to_array( $groups ) {
		if ( is_array( $groups ) ) {
			$group_ids = array();
	...
	public function bm_get_group_admin_label( $gid ) {
		$dbhandler   = new bm_DBhandler();
		$group       = $dbhandler->get_row( 'GROUPS', $gid );
		...
		return apply_filters( 'bm_filter_group_admin_label', $admin_label, $gid );
	}

	public function bm_get_all_group_blogs( $gid, $pagenum = 1, $limit = 10, $sort_by = 'title-asc', $search_in = 'post_title', $search = '' ) {
		$dbhandler  = new bm_DBhandler();
		$meta_query = array(
			'relation' => 'AND',
			array(
		...
		$users      = $dbhandler->bm_get_all_users( '', $meta_query );
		$author_ids = array( '0' );
		...
		$pagination = $dbhandler->bm_get_pagination( $num_of_pages, $pagenum );
		?>
...
<table class="pg-group-members pg-blog-setting">
	<tbody>
		...
		<!--********************************-->
			...
			<?php foreach ( $posts_array as $post ) : ?>
				<?php
				$bm_admin_note_content = trim( get_post_meta( $post->ID, 'bm_admin_note_content', true ) );
				$is_user_active        = $this->bm_is_user_active( $post->post_author );
				?>
		<tr>
			<td><input class="pg-blog-checked <?php
				...
				?>" type="checkbox" value="<?php echo esc_attr( $post->ID ); ?>" name="pg_edit_blog_id[]" onclick="bm_show_hide_batch_operation('blog')"></td>
			<td>
			...
			<td><?php echo wp_kses_post( $this->bm_change_date_in_different_format( $post->post_modified ) ); ?></td>
			<td><?php echo wp_kses_post( $this->bm_get_author_name_and_link( $post->post_author ) ); ?></td>
			<td>
				<?php
				if ( $bm_admin_note_content != '' ) {
					?>
		...
		if ( trim( $search ) != '' ) :
			switch ( $search_in ) {
				...
				case 'author_name':
					...
					$users      = $dbhandler->bm_get_all_users( $search, $meta_query );
	...
	public function bm_change_date_in_different_format( $date, $tab = '' ) {
	...
	public function bm_is_user_active( $uid ) {
	...
	public function bm_get_author_name_and_link( $uid ) {
		$user      = get_user_by( 'ID', $uid );
		$name      = $user->display_name;
		$url       = $this->bm_get_user_profile_url( $uid );
		$is_active = $this->bm_is_user_active( $uid );
		if ( $is_active ) {
	...
	public function bm_get_all_users_from_group( $gid, $pagenum = 1, $limit = 10, $sort_by = 'first_name_asc', $search_in = 'user_login', $search = '' ) {
		$dbhandler = new bm_DBhandler();
		$offset    = ( $pagenum - 1 ) * $limit;
		$get       = array( 'gid' => $gid );
		if ( trim( $search ) != '' ) {
		...
		$meta_query_array = $this->bm_get_user_meta_query( $get );

		$user_query   = $dbhandler->bm_get_all_users_ajax( $search, $meta_query_array, '', $offset, $limit, $order, $sortby, array( $current_user->ID ) );
...
<input type="hidden" name="pg_total_group_members_sigle_page" id="pg_total_group_members_sigle_page" value="<?php echo esc_attr( $limit ); ?>" />
 <table class="pg-group-members">
	<tbody>
		...
		<tr>
			...
			<td><input class="pg-member-checked <?php
				...
				?>" type="checkbox"   value="<?php echo esc_attr( $user->ID ); ?>" name=""onclick="bm_show_hide_batch_operation('member')"></td>
			<td>
				<div class="pg-member-avatar">
					<a href="<?php echo esc_url( $this->bm_get_user_profile_url( $user->ID ) ); ?>" target="_blank">
						<?php echo get_avatar( $user->user_email, 26, '', false, array( 'force_display' => true ) ); ?>
					</a>
				</div>
			</td>
			<td class="pg-group-member-name"><a href="<?php echo esc_url( $this->bm_get_user_profile_url( $user->ID ) ); ?>" target="_blank"><?php echo esc_html( $user->first_name ); ?></a></td>
			<td class="pg-group-member-name"><a href="<?php echo esc_url( $this->bm_get_user_profile_url( $user->ID ) ); ?>" target="_blank"><?php echo esc_html( $user->last_name ); ?></a></td>
			
			<td><?php $login_status = ( $this->bm_get_user_online_status( $user->ID ) == 1 ? 'pm-online' : 'pm-offline' ); ?> <span class="pm-friend-status <?php echo esc_attr( $login_status ); ?>"></span></td>
						...
						<td>     
							<div class="pg-setting-dropdown" onclick="pg_toggle_dropdown_menu(this)">
								...
								<ul class="pg-dropdown-menu">
									...
									<?php $is_user_active = $this->bm_is_user_active( $user->ID ); if ( $is_user_active ) : ?>
									...
									<li><a href="<?php echo esc_url( $this->bm_get_user_edit_profile_url( $user->ID, $gid ) ); ?>"><?php esc_html_e( 'Edit Profile', 'profilegrid-user-profiles-groups-and-communities' ); ?></a></li>
									<?php else : ?>
									...
									<?php endif; ?>
									 <?php do_action( 'bm_cog_option', $user->ID, $gid ); ?>
								</ul>
							</div>
						</td>
		</tr>
	...
	public function bm_get_all_users_from_group_grid_view( $gid, $pagenum = 1, $limit = 10, $sort_by = 'first_name_asc', $search_in = 'user_login', $search = '', $profile_magic = 'profilegrid-user-profiles-groups-and-communities', $pg_version = '5.1.3' ) {
		$dbhandler     = new bm_DBhandler();
		$pmhtmlcreator = new bm_HTML_Creator( $profile_magic, $pg_version );
		$leaders       = array();
		...
		$hide_users   = $this->bm_get_hide_users_array();
		$user_query   = $dbhandler->bm_get_all_users_ajax( $search, $meta_query_array, '', $offset, $limit, $order, $sortby, $hide_users );
		$total_users  = $user_query->get_total();
		$users        = $user_query->get_results();
		$num_of_pages = ceil( $total_users / $limit );
		$pagination   = $dbhandler->bm_get_pagination( $num_of_pages, $pagenum );
...
<input type="hidden" name="pg_total_group_members_sigle_page_grid" id="pg_total_group_members_sigle_page_grid" value="<?php echo esc_attr( $limit ); ?>" />
	...
	public function generate_members_args( $args, $sort_by, $search_in, $search, $gid ) {
		$dbhandler  = new bm_DBhandler();
		$get        =
		$meta_query = $this->bm_get_user_meta_query( $get );
		switch ( $sort_by ) {
			case 'name_asc':
				$args['orderby'] = 'login';
				$args['order']   = 'ASC';
	...
	public function pg_get_user_groups_badge_slider( $uid ) {
		...
		$user_groups = $this->profile_magic_get_user_field_value( $uid, 'bm_group' );
		...
		if ( ! empty( $gid_array ) ) {
			...
			foreach ( $gid_array as $gid ) {
				if ( $i >= 4 ) {
					continue;}
				$group_page_url  = $this->profile_magic_get_frontend_url( 'bm_group_page', '', $gid );
	...
	public function pg_leave_group( $uid, $gid ) {
		$dbhandler   = new bm_DBhandler();
		$bm_emails   = new bm_Emails();
		$user_group  = $this->profile_magic_get_user_field_value( $uid, 'bm_group' );
		...
		if ( $is_leader ) {
			...
			$group_page_url      = admin_url( 'admin.php?page=bm_add_group' );
			...
			$message  = $bm_emails->bm_filter_email_content( $message, $uid, false, $gid );
			$bm_emails->bm_send_admin_notification( $subject, $message );
		}
		...
		$update = update_user_meta( $uid, 'bm_group', $gid_array );
		if ( $update ) {
	...
	public function pg_unassign_group_during_delete_group( $uid, $gid ) {
		...
		if ( ! empty( $gid_array ) ) {
			update_user_meta( $uid, 'bm_group', $gid_array );
		} else {
			delete_user_meta( $uid, 'bm_group' );
		}
	...
	public function profile_magic_check_is_group_member( $gid, $uid ) {
		 $user_group = $this->profile_magic_get_user_field_value( $uid, 'bm_group' );
		$bm_group    = $this->pg_filter_users_group_ids( $user_group );
		if ( is_array( $bm_group ) && ! empty( $bm_group ) ) {
			if ( in_array( $gid, $bm_group ) ) {
				return true;
			} else {
				return false;
			}
	...
	public function profile_magic_get_join_group_button( $gid, $show_action = 'no' ) {
		...
		$registration_url = add_query_arg( 'gid', $gid, $registration_url );
		if ( $show_action == 'yes' ) {
			$group_page = $this->profile_magic_get_frontend_url( 'bm_group_page', '' );
		...
		$meta_query_array     = $pmrequests->bm_get_user_meta_query( array( 'gid' => $gid ) );
		$limit                = $dbhandler->get_value( 'GROUPS', 'group_limit', $gid );
		$total_users_in_group = count( $dbhandler->bm_get_all_users( '', $meta_query_array ) );
		if ( is_user_logged_in() ) {
			$current_user = wp_get_current_user();
			...
			if ( $requested == null ) {
				if ( $this->profile_magic_check_is_group_member( $gid, $current_user->ID ) ) {
					if ( $dbhandler->get_global_option_value( 'bm_show_group_leave_group_button', '1' ) ) :
						?>
					<div class="pm-group-signup">
				   <a class="bm_button" onclick="pg_edit_blog_popup('group','remove_group','<?php echo esc_attr( $current_user->ID ); ?>','<?php echo esc_attr( $gid ); ?>')">
						...
						if ( $limit > $total_users_in_group ) {
							...
							<div class="pm-group-signup">
								<form method="post" <?php echo wp_kses_post( $action ); ?>>
									...
									<a class="bm_button" onclick="submit()">
			...
			if ( $is_group_limit == 1 ) {
				if ( $limit > $total_users_in_group ) {
					?>
				<div class="pm-group-signup">
					<a href="<?php echo esc_url( $registration_url ); ?>" class="bm_button">
	...
	public function profile_magic_join_group_fun( $uid, $gid, $type ) {
		 $pmemails    = new bm_Emails();
		$dbhandler    = new bm_DBhandler();
		$notification = new Profile_Magic_Notification();
		if ( $type == 'open' ) {
			$user_group    = maybe_unserialize( $this->profile_magic_get_user_field_value( $uid, 'bm_group' ) );
			$user_groups   = $this->pg_filter_users_group_ids( $user_group );
			$joining_dates = $this->profile_magic_get_user_field_value( $uid, 'bm_joining_date' );
			...
			$update     = update_user_meta( $uid, 'bm_joining_date', $joining_dates );
			$update     = update_user_meta( $uid, 'bm_group', $gid_array );
			$where      = array(
				'gid' => $gid,
				'uid' => $uid,
			...
			$pmemails->bm_send_group_based_notification( $gid, $uid, 'on_registration' );
			$notification->bm_joined_new_group_notification( $uid, $gid );
			return $update;
		} else {
			...
			if ( ! empty( $groupleaders ) ) :
				foreach ( $groupleaders as $leader ) {
					 ...
					 $pmemails->bm_send_group_based_notification_to_group_admin( $gid, $uid, 'on_membership_request', $leader );
				}
			endif;
	...
	public function bm_get_all_join_group_requests( $gid, $pagenum = 1, $limit = 10, $sort_by = 'first_name_asc', $search = '', $search_in = 'user_login' ) {
		...
		if ( ! empty( $get ) ) {
			$meta_query_array = $this->bm_get_user_meta_query( $get ); }
		$user_query   = $dbhandler->bm_get_all_users_ajax( $search, $meta_query_array, '', $offset, $limit, $order, $sortby, array( $current_user->ID ), array(), $user_request );
		$total_users  = $user_query->get_total();
		$users        = $user_query->get_results();
		$num_of_pages = ceil( $total_users / $limit );
		$pagination   = $dbhandler->bm_get_pagination( $num_of_pages, $pagenum );
...
<table class="pg-group-requests pg-group-members">
	<tbody>
		...
		<!--********************************-->
			...
			foreach ( $users as $user ) :
				...
				?>
		<tr>
			<td><input class="pg-request-checked" type="checkbox"   value="<?php echo esc_attr( $user->ID ); ?>" name=""onclick="bm_show_hide_batch_operation('requests')"></td>
			<td>
				<div class="pg-member-avatar">
					<a href="<?php echo esc_url( $this->bm_get_user_profile_url( $user->ID ) ); ?>" target="_blank">
						<?php echo get_avatar( $user->user_email, 26, '', false, array( 'force_display' => true ) ); ?>
			...
			<td class="pg-group-member-name"><a href="<?php echo esc_url( $this->bm_get_user_profile_url( $user->ID ) ); ?>" target="_blank"><?php echo esc_html( $user->last_name ); ?></a></td>
			<td><?php $login_status = ( $this->bm_get_user_online_status( $user->ID ) == 1 ? 'pm-online' : 'pm-offline' ); ?> <span class="pm-friend-status <?php echo esc_attr( $login_status ); ?>"></span></td>
			<td><?php echo wp_kses_post( $this->bm_change_date_in_different_format( $request_options['request_date'], 'request' ) ); ?></td>
	...
	public function bm_get_all_rm_registration_form_dropdown_list( $selected ) {
		$dbhandler = new bm_DBhandler();
	...
	public function bm_get_all_rm_registration_form_fields_dropdown_list( $selected, $form_id ) {
		$dbhandler = new bm_DBhandler();
	...
	public function bm_check_if_group_associate_with_rm_form( $gid ) {
		$dbhandler     = new bm_DBhandler();
		...
		if ( isset( $group_options ) && isset( $group_options['pg_rm_form'] ) && $group_options['pg_rm_form']!=0 ) {
			$form_type = $this->bm_check_rm_form_type( $group_options['pg_rm_form'] );
			if ( $form_type ) {
	...
	public function bm_check_rm_form_type( $form_id ) {
		 $dbhandler = new bm_DBhandler();
		$type       = $dbhandler->get_value( 'FORMS', 'form_type', $form_id, 'form_id' );
		return $type;
	}

	public function bm_check_rm_form_associate_with_groups( $form_id ) {
		$dbhandler        = new bm_DBhandler();
	...
	public function bm_get_map_fields_with_rm_form( $gid ) {
		$dbhandler = new bm_DBhandler();
		$mapping   = array();
	...
	public function bm_check_group_limit( $gid ) {
		$dbhandler            = new bm_DBhandler();
		$message              = '';
		$limit                = $dbhandler->get_value( 'GROUPS', 'group_limit', $gid );
		$is_group_limit       = $dbhandler->get_value( 'GROUPS', 'is_group_limit', $gid );
		$meta_query_array     = $this->bm_get_user_meta_query( array( 'gid' => $gid ) );
		$total_users_in_group = count( $dbhandler->bm_get_all_users( '', $meta_query_array ) );
		if ( $is_group_limit == 1 ) {
			if ( $limit > $total_users_in_group ) {
					$message = '';
	...
	public function pg_get_group_joining_date( $gid, $uid ) {
		...
		return $this->bm_change_date_in_different_format( $date );
	}
	...
	public function bm_get_all_groups_data( $view = 'grid', $pagenum = 1, $limit = 10, $sort_by = 'newest', $search = '' ) {
		$dbhandler = new bm_DBhandler();
		$offset    = ( $pagenum - 1 ) * $limit;
		...
		$additional = apply_filters( 'bm_get_all_groups_data_additional', $additional );
		...
		if ( ! empty( $groups ) ) {
			foreach ( $groups as $group ) :
				$this->bm_get_group_html( $group, $view );
			endforeach;
	...
	public function bm_get_group_html( $group, $view = 'grid' ) {
		$dbhandler = new bm_DBhandler();
		$option    = 'bm_show_group_on_groups_page_' . $group->id;
		if ( $dbhandler->get_global_option_value( $option, '1' ) == '1' ) {
			if ( $view == 'grid' ) {

				$group_url        = $this->profile_magic_get_frontend_url( 'bm_group_page', '', $group->id );
				...
				<div class="pm-group pm-difl pm-border pm-radius5 pm-bg-lt pm50">
				  ...
				  <div class="pm-group-button pm-dbfl pm-pad10">
				   <div class="pm-group-signup"><a href="<?php echo esc_url( $registration_url ); ?>" class="bm_button"><button><?php esc_html_e( 'Join Group', 'profilegrid-user-profiles-groups-and-communities' ); ?></button></a></div>
						<?php if ( $this->profile_magic_check_paid_group( $group->id ) > 0 ) : ?>
					<div class="bm_group_price">
							<?php
							if ( $dbhandler->get_global_option_value( 'bm_currency_position', 'before' ) == 'before' ) :
								echo wp_kses_post( $this->bm_get_currency_symbol() . ' ' . $this->profile_magic_check_paid_group( $group->id ) );
						else :
							echo wp_kses_post( $this->profile_magic_check_paid_group( $group->id ) . ' ' . $this->bm_get_currency_symbol() );
						endif;
						?>
					</div>
					<?php else : ?>
					<div class="bm_free_group"><?php esc_html_e( 'Free', 'profilegrid-user-profiles-groups-and-communities' ); ?></div>
					<?php endif; ?>
					</div>
				   <?php else : ?>
				  <div class="pm-group-button pm-dbfl pm-pad10">
				   <div class="pm-group-signup">
									   <a href="<?php echo esc_url( $group_url ); ?>" class="bm_button">
		 ...
		 <div class="pm-group-list-view pm-dbfl">
			 ...
			 <div class="pm-group-name-desc pm-difl">
				 ...
				 <?php
					...
					<div class="pm-group-list-view-info"><?php echo esc_html( $group_type ); ?> / <a> <span><?php echo esc_html( count( $group_leaders ) ); ?></span></a> <?php echo esc_html( $this->bm_get_group_admin_label( $group->id ) ); ?> / <a><span><?php echo esc_html( $this->bm_count_group_members( $group->id ) ); ?></span></a> <?php esc_html_e( 'Members', 'profilegrid-user-profiles-groups-and-communities' ); ?></div>
			 </div>
	...
	public function bm_count_group_members( $gid ) {
		$dbhandler        = new bm_DBhandler();
		$meta_query_array = $this->bm_get_user_meta_query( array( 'gid' => $gid ) );
		$user_query       = $dbhandler->bm_get_all_users_ajax( '', $meta_query_array );
		$total_users      = $user_query->get_total();
		return $total_users;
	}

	public function bm_show_profile_image_profile_page( $uid, $cuid, $user_info ) {
		 $dbhandler = new bm_DBhandler();
		$id         = ( $uid != $cuid ) ? 'id="pm-show-profile-image"' : '';
		if ( $dbhandler->get_global_option_value( 'bm_show_profile_image', '1' ) == '1' ) {
			...
			if ( $uid == $cuid && $dbhandler->get_global_option_value( 'bm_show_change_profile_image_option', '1' ) == '1' ) :
			...
			$html .= '</div>';
			return apply_filters( 'bm_show_profile_image_html', $html, $uid, $cuid, $user_info );
		} else {
	...
	public function bm_profile_tabs() {
		 $dbhandler               = new bm_DBhandler();
		$tabs                     = array();
		$blog_tab_status          = $dbhandler->get_global_option_value( 'bm_enable_blog', '1' );
		$message_tab_status       = $dbhandler->get_global_option_value( 'bm_enable_private_messaging', '1' );
		$friends_tab_status       = $dbhandler->get_global_option_value( 'bm_friends_panel', '0' );
		$tabs['pg-about']         = array(
			'id'     => 'pg-about',
		...
		$bm_profile_tabs_order_status = maybe_unserialize( $dbhandler->get_global_option_value( 'bm_profile_tabs_order_status', $tabs ) );
		return apply_filters( 'bm_profile_tabs', $bm_profile_tabs_order_status );
	}
		...
		if ( isset( $tab ) && $tab['status'] == '1' ) {
			switch ( $id ) {
				...
				case 'pg-notifications':
					if ( $uid == $current_user->ID ) {
						$bm_notification     = new Profile_Magic_Notification();
						$unread_notification = $bm_notification->bm_get_user_unread_notification_count( $current_user->ID );
	...
	public function pg_group_card_html( $gid ) {
		...
		$user_query  = $dbhandler->bm_get_all_users_ajax( '', $meta_query, '', $offset, $limit, 'ASC', 'ID', $hide_users );
		$total_users = $user_query->get_total();
		$leaders     = array();
		if ( $row->is_group_leader != 0 ) {
			$leaders = $this->pg_get_group_leaders( $gid );
		...
		?>
		 ...
		 <div class="pm-group-card pm-dbfl pm-border pm-bg pm-radius5">
			...
			if ( is_user_logged_in() && in_array( $current_user->ID, $leaders ) ) :
				$edit_group = $this->profile_magic_get_frontend_url( 'bm_group_page', '', $gid );
				$edit_group = add_query_arg( 'gid', $gid, $edit_group );
				$edit_group = add_query_arg( 'edit', '1', $edit_group );
				?>
		   <div class="pm-edit-group"><a href="<?php echo esc_url( $edit_group ); ?>" class="bm_button"><?php esc_html_e( 'Edit', 'profilegrid-user-profiles-groups-and-communities' ); ?></a></div>
			<?php endif; ?>
			</div>
			 ...
			 <div class="pm-group-description pm-difl pm-bg pm-pad10 pm-border">
				...
				if ( ! class_exists( 'Profilegrid_Group_Multi_Admins' ) ) :
					if ( isset( $leaders ) && ! empty( $leaders ) && $pagenum == 1 ) :
                                                ...
                                                $profile_url = $this->bm_get_user_profile_url($leaders['primary']);

						?>
								<div class="pm-card-row pm-dbfl">
																	<div class="pm-card-label pm-difl"><?php echo esc_html( $this->bm_get_group_admin_label( $gid ) ); ?></div>
										 ...
										 <a href="<?php echo esc_url( $profile_url ); ?>                                								 <?php
											echo get_avatar(
												...
												array(
													...
													?>
								 <?php echo wp_kses_post( $this->bm_get_display_name( $leaders['primary'], true ) ); ?>
										 </a>
						...
						<?php
								...
								do_action( 'bm_show_multi_admins', $gid );
							endif;
	...
	public function bm_check_user_exist_by_id( $user_id ) {
		 $dbhandler   = new bm_DBhandler();
		$current_user = wp_get_current_user();
		$user         = get_user_by( 'ID', $user_id );
		$access       = $this->profile_magic_check_profile_access_permission( $user_id );
		if ( $dbhandler->get_global_option_value( 'bm_enable_private_profile' ) == '1' ) {
